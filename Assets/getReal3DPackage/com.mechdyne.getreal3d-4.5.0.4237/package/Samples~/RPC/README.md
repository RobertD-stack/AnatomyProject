...
This example shows how to setup and use Remote Procedure Calls (RPCs). RPCs are
useful when an action on the master must affect all the render nodes.
...
- Add the `RpcExample` component to the root object of the `Canvas`.

- Assign the sphere to the `RpcExample` targetRenderer field using drag and
  drop.
