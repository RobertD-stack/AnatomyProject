...
{
    /// <summary>
    /// CharacterMotorC is used to handle walking, jumping and falling of the player.
    /// </summary>
    [RequireComponent(typeof(CharacterController))]
    public class CharacterMotorC : MonoBehaviour
    {
        /// Movement transfer type on jump
        [System.Serializable]
        ...
        {
            [Tooltip("If moving platform is enabled")]
            public bool enabled = true;

            [Tooltip("Movement transfer type when jumping")]
        ...
        {
            [Tooltip("The maximum horizontal forward speed when moving")]
            public float maxForwardSpeed = 10.0f;

            [Tooltip("The maximum horizontal sideway speed when moving")]
            public float maxSidewaysSpeed = 10.0f;

            [Tooltip("The maximum horizontal backward speed when moving")]
            public float maxBackwardsSpeed = 10.0f;

            [Tooltip("Curve for multiplying speed based on slope (negative = downwards)")]
            public AnimationCurve slopeSpeedMultiplier = new AnimationCurve(new Keyframe(-90, 1), new Keyframe(0, 1), new Keyframe(90, 0));

            [Tooltip("How fast does the character change speeds when grounded? Higher is faster.")]
            public float maxGroundAcceleration = 30.0f;

            [Tooltip("How fast does the character change speeds when in the air? Higher is faster.")]
            public float maxAirAcceleration = 20.0f;

            [Tooltip("The gravity for the character")]
            public float gravity = 10.0f;

            [Tooltip("The maximum fall speed of the character")]
            ...
            /// Current hit point of the capsule collider
            [System.NonSerialized]
            public Vector3 hitPoint = Vector3.zero;

            /// Last hit point of the capsule collider
            [System.NonSerialized]
            public Vector3 lastHitPoint = new Vector3(Mathf.Infinity, 0, 0);

            internal void OnValidate()
            {
                maxForwardSpeed = Mathf.Max(0.0001f, maxForwardSpeed);
                maxSidewaysSpeed = Mathf.Max(0.0001f, maxSidewaysSpeed);
                maxBackwardsSpeed = Mathf.Max(0.0001f, maxBackwardsSpeed);
                maxGroundAcceleration = Mathf.Max(0f, maxGroundAcceleration);
                maxAirAcceleration = Mathf.Max(0f, maxAirAcceleration);
        ...
        public class CharacterMotorJumping
        {
            ...
            /// Time of the last jump button down
            [System.NonSerialized]
            ...
            internal void OnValidate()
            {
                ...
                perpAmount = Mathf.Max(0f, perpAmount);
                steepPerpAmount = Mathf.Max(0f, steepPerpAmount);
            }
        }

        /// Sliding related parameters
        [System.Serializable]
        public class CharacterMotorSliding
        {
            ...
            internal void OnValidate()
            {
                slidingSpeed = Mathf.Max(0f, slidingSpeed);
                sidewaysControl = Mathf.Max(0f, sidewaysControl);
                speedControl = Mathf.Max(0f, speedControl);
            }
        }
        ...
        [Tooltip("The current global direction we want the character to move in.")]
        [System.NonSerialized]
        public Vector3 inputMoveDirection = Vector3.zero;

        [Tooltip("Is the jump button held down? We use this interface instead of checking for the jump button directly so this script can also be used by AIs.")]
        [System.NonSerialized]
        ...
        [Tooltip("Moving platform settings")]
        public CharacterMotorMovingPlatform movingPlatform = new CharacterMotorMovingPlatform();

        [Tooltip("Sliding settings")]
        public CharacterMotorSliding sliding = new CharacterMotorSliding();

        [System.NonSerialized]
        private bool m_grounded = true;

        /// If the character is currently touching the ground
