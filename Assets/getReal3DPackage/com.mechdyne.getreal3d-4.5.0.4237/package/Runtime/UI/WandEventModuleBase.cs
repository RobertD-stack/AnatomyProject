...
namespace getReal3D
{
    ...
    public abstract class WandEventModuleBase : PointerInputModule
    {
        ...
        [Tooltip("If this module sends events from the wand")]
        ...
        [Tooltip("If the wand move events are sent")]
        public bool m_sendMoveEvents = false;

        [Tooltip("Number of move event to send per second")]
        public float m_inputActionsPerSecond = 10f;

        [Tooltip("Move event repeat delay")]
        public float m_repeatDelay = 0.5f;

        /// Modes of master mouse interaction
        ...
        /// Button functions (used to query for push/release on the derived classes)
        ...
        [Tooltip("What kind of mouse interaction is supported on the getReal3D master node?")]
        public MouseInteractionMode m_mouseInteractionMode = MouseInteractionMode.OverlayOnly;

        [Tooltip("What is the master camera?")]
        ...
        /// Get the gameobject that is currenlty pointed by the wand.
        public GameObject currentLook
        {
            get { return wand.pointerCurrentRaycast.gameObject; }
        }
        ...
        private void GetMousePointerEventData(PointerEventData pointerEventData)
        {
            ...
            {
                BuildPointerEventData(pointerEventData, Mouse.current.position.value); // no gr3d warning
            }
#elif ENABLE_LEGACY_INPUT_MANAGER
        BuildPointerEventData(pointerEventData, UnityEngine.Input.mousePosition); // no gr3d warning
#else
#warning Uhh?
#endif
        ...
        private List<UnityEngine.EventSystems.BaseRaycaster> GetRaycasters()
        {
            ...
            {
                return Plugin.FindObjectsByTypeUnsorted<BaseRaycaster>().ToList();
            }
            else
            {
        ...
        private void replaceCanvasCameras(Camera camera)
        {
            ...
            m_camerasBackup = new Camera[raycasters.Count];
            ...
            for (int i = 0; i < raycasters.Count; i++)
            {
                ...
                // Check if the canvas exists before doing anything
                if (canvas != null)
                {
                    m_camerasBackup[i] = canvas.worldCamera;
                    canvas.worldCamera = camera;
                }
                else
                {
                    // If there is no canvas, just set the backup slot to null
                    m_camerasBackup[i] = null;
                }
            }
        }
        ...
        private void restoreCanvasCameras()
        {
            ...
            for (int i = 0; i < raycasters.Count; ++i)
            {
                var canvas = raycasters[i].GetComponent<Canvas>();

                // Check if the canvas exists before trying to access .worldCamera
                if (canvas != null)
                {
                    canvas.worldCamera = m_camerasBackup[i];
                }
            }
        }
        ...
        /// Process inputs and send corresponding events
        public override void Process()
        {
            ...
            if (m_mouseInteractionMode != MouseInteractionMode.None)
            {
                ...
                GetMousePointerEventData(mouse);
#if ENABLE_INPUT_SYSTEM
                if (Mouse.current != null)
                {
                    ProcessMousePress(mouse, Mouse.current.leftButton.wasPressedThisFrame, Mouse.current.leftButton.wasReleasedThisFrame); // no gr3d warning
                }
#elif ENABLE_LEGACY_INPUT_MANAGER
            ProcessMousePress(m_mouse, UnityEngine.Input.GetMouseButtonDown(0), UnityEngine.Input.GetMouseButtonUp(0)); // no gr3d warning
#else
#warning Uhh?
#endif
                ProcessMove(mouse);
            ...
            if (m_sendWandEvents)
            {
                GetWandPointerEventData(wand);
                ProcessMousePress(wand,
        ...
        /// Return true if the button corresponding to the given function was released during this frame.
        protected abstract bool GetButtonUp(ButtonFunction function);

        /// Return true if the button corresponding to the given function was pushed during this frame.
        protected abstract bool GetButtonDown(ButtonFunction function);

        /// Return the 2D navigation vector.
        ...
        /// Process dragging of the mouse or wand
