...
namespace getReal3D
{
    ...
    {
        /// Return true to prevent the menu to show up.
    ...
    public class GenericShowMenu : MonoBehaviour, MenuClickedOustideCallback
    {
        ...
        void Update()
        {
            ...
            {
                if (playerInputs.WandButtonDown && !m_menuObject.gameObject.activeSelf &&
                    !m_preventsShowMenu.Any(p => p.preventsShowMenu(m_wandObject)))
                {
                    if (m_wandEventModule)
                    {
                        m_wandEventModule.playerInputs = playerInputs;
                        ...
                        m_menuObject.GetComponentsInChildren<GenericMenuDrag>())
                    {
                        ...
                        menuDrag.m_inputs = playerInputs;
                    }
        ...
        /// Called by GenericMenuDrag when the wand button is pressed while
        /// the wand is not pointing at the menu. This will hide the menu.
        ...
        /// Reset the menu position to its default. This depends on both the menu spawn reference
        /// and the default menu distance.
